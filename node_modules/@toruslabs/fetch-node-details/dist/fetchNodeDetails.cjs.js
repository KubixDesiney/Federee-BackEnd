...
class NodeDetailManager {
  constructor({
    ...
    sigType = constants_namespaceObject.SIG_TYPE.ECDSA_SECP256K1,
    fndServerEndpoint,
    enableLogging = false
  } = {}) {
    ...
    defineProperty_default()(this, "_sigType", void 0);
    ...
    if (network && !Object.values(objectSpread2_default()(objectSpread2_default()({}, constants_namespaceObject.TORUS_LEGACY_NETWORK), constants_namespaceObject.TORUS_SAPPHIRE_NETWORK)).includes(network)) {
  ...
  async getNodeDetails({
    ...
    try {
      ...
      try {
        const {
          nodeDetails
        } = await (0,http_helpers_namespaceObject.get)(`${this.fndServerEndpoint}?network=${this.network}&verifier=${verifier}&verifierId=${verifierId}&keyType=${this._keyType}&sigType=${this._sigType}`);
        this.setNodeDetails(nodeDetails);
        return this._nodeDetails;
      } catch (error) {
        log.error("Failed to fetch node details from server, using local.", error);
      }
      const nodeDetails = (0,fnd_base_namespaceObject.fetchLocalConfig)(this.network, this._keyType, this._sigType);
