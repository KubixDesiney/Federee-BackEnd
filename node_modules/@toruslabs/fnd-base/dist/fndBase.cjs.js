/******/ (() => { // webpackBootstrap
/******/ 	"use strict";
/******/ 	// The require scope
/******/ 	var __webpack_require__ = {};
/******/ 	
/************************************************************************/
/******/ 	/* webpack/runtime/define property getters */
/******/ 	(() => {
/******/ 		// define getter functions for harmony exports
/******/ 		__webpack_require__.d = (exports, definition) => {
/******/ 			for(var key in definition) {
/******/ 				if(__webpack_require__.o(definition, key) && !__webpack_require__.o(exports, key)) {
/******/ 					Object.defineProperty(exports, key, { enumerable: true, get: definition[key] });
/******/ 				}
/******/ 			}
/******/ 		};
/******/ 	})();
/******/ 	
/******/ 	/* webpack/runtime/hasOwnProperty shorthand */
/******/ 	(() => {
/******/ 		__webpack_require__.o = (obj, prop) => (Object.prototype.hasOwnProperty.call(obj, prop))
/******/ 	})();
/******/ 	
/******/ 	/* webpack/runtime/make namespace object */
/******/ 	(() => {
/******/ 		// define __esModule on exports
/******/ 		__webpack_require__.r = (exports) => {
/******/ 			if(typeof Symbol !== 'undefined' && Symbol.toStringTag) {
/******/ 				Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' });
/******/ 			}
/******/ 			Object.defineProperty(exports, '__esModule', { value: true });
/******/ 		};
/******/ 	})();
/******/ 	
/************************************************************************/
var __webpack_exports__ = {};
// ESM COMPAT FLAG
__webpack_require__.r(__webpack_exports__);

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  SAPPHIRE_NETWORK_URLS: () => (/* reexport */ SAPPHIRE_NETWORK_URLS),
  fetchLocalConfig: () => (/* reexport */ fetchLocalConfig),
  getRSSEndpoints: () => (/* reexport */ getRSSEndpoints),
  getSSSEndpoints: () => (/* reexport */ getSSSEndpoints),
  getSapphireNodeDetails: () => (/* reexport */ getSapphireNodeDetails),
  getTSSEndpoints: () => (/* reexport */ getTSSEndpoints)
});

;// external "@toruslabs/constants"
const constants_namespaceObject = require("@toruslabs/constants");
;// ./src/endpoints.ts

const SAPPHIRE_NETWORK_URLS = {
  [constants_namespaceObject.TORUS_SAPPHIRE_NETWORK.SAPPHIRE_DEVNET]: ["https://node-1.dev-node.web3auth.io", "https://node-2.dev-node.web3auth.io", "https://node-3.dev-node.web3auth.io", "https://node-4.dev-node.web3auth.io", "https://node-5.dev-node.web3auth.io"],
  [constants_namespaceObject.TORUS_SAPPHIRE_NETWORK.SAPPHIRE_MAINNET]: ["https://node-1.node.web3auth.io", "https://node-2.node.web3auth.io", "https://node-3.node.web3auth.io", "https://node-4.node.web3auth.io", "https://node-5.node.web3auth.io"]
};
const getSSSEndpoints = (sapphireNetwork, legacyNetwork) => {
  const endpoints = SAPPHIRE_NETWORK_URLS[sapphireNetwork];
  if (!endpoints || endpoints.length === 0) {
    throw new Error(`Unsupported network: ${sapphireNetwork}`);
  }
  const routeIdentifier = constants_namespaceObject.LEGACY_NETWORKS_ROUTE_MAP[legacyNetwork];
  return endpoints.map(e => {
    if (routeIdentifier && routeIdentifier.networkIdentifier) {
      return `${e}/sss/${routeIdentifier.networkIdentifier}/jrpc`;
    }
    return `${e}/sss/jrpc`;
  });
};
const getRSSEndpoints = (sapphireNetwork, legacyNetwork) => {
  const endpoints = SAPPHIRE_NETWORK_URLS[sapphireNetwork];
  if (!endpoints || endpoints.length === 0) {
    throw new Error(`Unsupported network: ${sapphireNetwork}`);
  }
  const routeIdentifier = constants_namespaceObject.LEGACY_NETWORKS_ROUTE_MAP[legacyNetwork];
  return endpoints.map(e => {
    if (routeIdentifier && routeIdentifier.networkIdentifier) {
      return `${e}/rss/${routeIdentifier.networkIdentifier}`;
    }
    return `${e}/rss`;
  });
};
const getTSSEndpoints = (sapphireNetwork, legacyNetwork, keyType = constants_namespaceObject.KEY_TYPE.SECP256K1, sigType) => {
  const endpoints = SAPPHIRE_NETWORK_URLS[sapphireNetwork];
  if (!endpoints || endpoints.length === 0) {
    throw new Error(`Unsupported network: ${sapphireNetwork}`);
  }
  const tssPath = (() => {
    const dklsPath = "tss";
    const frostPath = "tss-frost";
    if (sigType) {
      if (sigType === constants_namespaceObject.SIG_TYPE.ECDSA_SECP256K1) {
        if (keyType !== constants_namespaceObject.KEY_TYPE.SECP256K1) {
          throw new Error("Invalid key type for ecdsa-secp256k1");
        }
        return dklsPath;
      } else if (sigType === constants_namespaceObject.SIG_TYPE.ED25519) {
        if (keyType !== constants_namespaceObject.KEY_TYPE.ED25519) {
          throw new Error("Invalid key type for ed25519");
        }
        return frostPath;
      } else if (sigType === constants_namespaceObject.SIG_TYPE.BIP340) {
        if (keyType !== constants_namespaceObject.KEY_TYPE.SECP256K1) {
          throw new Error("Invalid key type for bip340");
        }
        return frostPath;
      }
      throw new Error("Invalid sig type");
    } else if (keyType === constants_namespaceObject.KEY_TYPE.SECP256K1) {
      return dklsPath;
    } else if (keyType === constants_namespaceObject.KEY_TYPE.ED25519) {
      return frostPath;
    }
    throw new Error("Invalid key type");
  })();
  const routeIdentifier = constants_namespaceObject.LEGACY_NETWORKS_ROUTE_MAP[legacyNetwork];
  return endpoints.map(e => {
    if (routeIdentifier && routeIdentifier.networkIdentifier) {
      return `${e}/${tssPath}/${routeIdentifier.networkIdentifier}`;
    }
    return `${e}/${tssPath}`;
  });
};
;// ./src/pubKeys.ts

const SAPPHIRE_NODE_PUB_KEYS = {
  [constants_namespaceObject.TORUS_SAPPHIRE_NETWORK.SAPPHIRE_MAINNET]: [{
    X: "e0925898fee0e9e941fdca7ee88deec99939ae9407e923535c4d4a3a3ff8b052",
    Y: "54b9fea924e3f3e40791f9987f4234ae4222412d65b74068032fa5d8b63375c1"
  }, {
    X: "9124cf1e280aab32ba50dffd2de81cecabc13d82d2c1fe9de82f3b3523f9b637",
    Y: "fca939a1ceb42ce745c55b21ef094f543b457630cb63a94ef4f1afeee2b1f107"
  }, {
    X: "555f681a63d469cc6c3a58a97e29ebd277425f0e6159708e7c7bf05f18f89476",
    Y: "606f2bcc0884fa5b64366fc3e8362e4939841b56acd60d5f4553cf36b891ac4e"
  }, {
    X: "2b5f58d8e340f1ab922e89b3a69a68930edfe51364644a456335e179bc130128",
    Y: "4b4daa05939426e3cbe7d08f0e773d2bf36f64c00d04620ee6df2a7af4d2247"
  }, {
    X: "3ecbb6a68afe72cf34ec6c0a12b5cb78a0d2e83ba402983b6adbc5f36219861a",
    Y: "dc1031c5cc8f0472bd521a62a64ebca9e163902c247bf05937daf4ae835091e4"
  }],
  [constants_namespaceObject.TORUS_SAPPHIRE_NETWORK.SAPPHIRE_DEVNET]: [{
    X: "f74389b0a4c8d10d2a687ae575f69b20f412d41ab7f1fe6b358aa14871327247",
    Y: "54e3a73098ed9bced3ef8821736e9794f9264a1420c0c7ad15d2fa617ba35ef7"
  }, {
    X: "bc38813a6873e526087918507c78fc3a61624670ee851ecfb4f3bef55d027b5a",
    Y: "ac4b21229f662a0aefdfdac21cf17c3261a392c74a8790db218b34e3e4c1d56a"
  }, {
    X: "b56541684ea5fa40c8337b7688d502f0e9e092098962ad344c34e94f06d293fb",
    Y: "759a998cef79d389082f9a75061a29190eec0cac99b8c25ddcf6b58569dad55c"
  }, {
    X: "7bcb058d4c6ffc6ba4bfdfd93d141af35a66338a62c7c27cdad2ae3f8289b767",
    Y: "336ab1935e41ed4719e162587f0ab55518db4207a1eb36cc72303f1b86689d2b"
  }, {
    X: "bf12a136ef94399ea098f926f04e26a4ec4ac70f69cce274e8893704c4951773",
    Y: "bdd44828020f52ce510e026338216ada184a6867eb4e19fb4c2d495d4a7e15e4"
  }]
};
;// ./src/sapphireNetworkConfig.ts


const getSapphireNodeDetails = (sapphireNetwork, legacyNetwork, keyType, sigType) => {
  return {
    currentEpoch: "1",
    torusNodeEndpoints: getSSSEndpoints(sapphireNetwork, legacyNetwork),
    torusNodeSSSEndpoints: getSSSEndpoints(sapphireNetwork, legacyNetwork),
    torusNodeRSSEndpoints: getRSSEndpoints(sapphireNetwork, legacyNetwork),
    torusNodeTSSEndpoints: getTSSEndpoints(sapphireNetwork, legacyNetwork, keyType, sigType),
    torusIndexes: [1, 2, 3, 4, 5],
    torusNodePub: SAPPHIRE_NODE_PUB_KEYS[sapphireNetwork]
  };
};
;// ./src/utils.ts


function fetchLocalConfig(network, keyType, sigType) {
  if (Object.values(constants_namespaceObject.TORUS_SAPPHIRE_NETWORK).includes(network)) {
    return getSapphireNodeDetails(network, undefined, keyType, sigType);
  }
  if (Object.values(constants_namespaceObject.TORUS_LEGACY_NETWORK).includes(network)) {
    const legacyMap = constants_namespaceObject.LEGACY_NETWORKS_ROUTE_MAP[network];
    if (legacyMap.migrationCompleted) return getSapphireNodeDetails(legacyMap.networkMigratedTo, network, keyType);
  }
  return undefined;
}
;// ./src/index.ts



module.exports = __webpack_exports__;
/******/ })()
;